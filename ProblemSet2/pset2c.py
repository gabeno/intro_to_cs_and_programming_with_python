...
system might be limited to an error message complaining about too much time taken.)
...
inputs in your solution to Problem 2 to compare!).
...
annualInterestRate = 0.18 # 0.2

monthly_interest_rate = annualInterestRate / 12.0
lo_monthly_payment = balance / 12
hi_monthly_payment = (balance * (1 + monthly_interest_rate)**12) / 12.0

while True:
    new_balance = balance
    monthly_payment = (lo_monthly_payment + hi_monthly_payment) / 2
    print 'lo: %s, hi: %s' % (lo_monthly_payment, hi_monthly_payment)
    print 'monthly payment: %s' % monthly_payment
    for month in range(1, 13):
        unpaid_balance = new_balance - monthly_payment
        interest_paid = monthly_interest_rate * unpaid_balance
        new_balance = unpaid_balance + interest_paid

    print 'balance: %s' % new_balance
    if new_balance > 0:
        lo_monthly_payment = monthly_payment
    else:
        hi_monthly_payment = monthly_payment
        if abs(new_balance) < 0.01:
            break

print 'Lowest Payment: %s' % round(monthly_payment, 2)

#balance: -0.0118134893374
#lo: 90325.0257595, hi: 90325.0267311
